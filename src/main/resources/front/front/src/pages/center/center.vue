	...
	<div class="center-preview">
		...
		<el-tabs class="center-tabs" tab-position="left" @tab-click="handleClick">
			...
			<el-tab-pane label="好友列表" v-if="changeHasChat()">
				<div class="z-box">
					<div class="section-content" v-for="item in friendList" :key="item.id" @click.stop="chatClick(item)">
						<div class="chat-left">
							<img :src="item.picture?baseUrl + item.picture:require('@/assets/avator.png')">
							<div class="chat-info">
								<div class="chat-name">{{item.name}}{{item.type==0?'申请添加您为好友':''}}</div>
							</div>
						</div>
						<div class="chat-btn">
							<div v-if="item.type==0" @click.stop="confirmFriend(item)" class="friendBtn1">同意</div>
							<div v-if="item.type==0" @click.stop="cancelFriend(item)" class="friendBtn2">拒绝</div>
							<div v-if="item.type==1" @click.stop="delFriend(item)" class="friendBtn2">删除</div>
						</div>
					</div>
...
<script>
	...
	export default {
		...
		created() {
			...
			if ('yonghu' == this.userTableName&&this.rules['touxiang']){
				this.rules['touxiang'].push({ required: true, message: '请输入头像', trigger: 'blur' })
			}else if('yonghu' == this.userTableName&&!this.rules['touxiang']) {
